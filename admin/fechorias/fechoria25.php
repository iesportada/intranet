		...
		// SMS
			if ($config['mod_sms']) {
				...
				if (($grave == "leve" or $grave == "grave" or $grave == "muy grave") and (substr ( $tfno, 0, 1 ) == "6" or substr ( $tfno, 0, 1 ) == "7" or substr ( $tfno_u, 0, 1 ) == "6" or substr ( $tfno_u, 0, 1 ) == "7")) {
					...
					$message = "Con fecha " .$fecha . ", en la ".$horaEnvia." hora, su hijo/a ".$nombre_alum." ha cometido una falta contra las normas de convivencia del Centro. Hable con él y, ante cualquier duda, consulte en http://".$config['dominio'];
					...
					// ENVIO DE SMS
						...
						echo "<div class=\"alert alert-error\"><strong>Error:</strong> No se pudo enviar el SMS al teléfono (+34) ".$mobile.". Corrija la información de contacto del alumno/a en Séneca e importe los datos nuevamente.
						</div><br>";
					}
				}
			}// FIN SMS
